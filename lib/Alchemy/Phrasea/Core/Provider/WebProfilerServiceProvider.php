...
class WebProfilerServiceProvider implements ServiceProviderInterface, EventListenerProviderInterface
{
    /**
    ...
    public function register(Container $app)
    {
        ...
        $app['web_profiler.toolbar.listener'] =
        ...
        if (class_exists(AjaxDataCollector::class)) {
            ...
            $app['data_collectors'] = $app->extend('data_collectors', function (array $collectors, $app) {
        ...
        $app['data_collectors.doctrine.logger'] = function ($app) {
            return new DbalLogger($app['logger'], $app['stopwatch']);
        };

        $app['data_collectors.doctrine'] = function ($app) {
            return new DoctrineDataCollector(new LazyArrayAccess(new LazyLocator($app, 'dbs')));
        };

        $app['cache'] = $app->extend('cache', function (Cache $cache, $app) {
        ...
        $app['data_collector.cache_subscriber'] = function ($app) {
    ...
    {
        $dispatcher->addSubscriber($app['data_collector.cache_subscriber']);
    }
}
